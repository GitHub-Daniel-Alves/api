...
public class ClientePessoaFisicaController {
    ...
    public ResponseEntity<ClientePessoaFisicaDTO> createCliente(@RequestBody ClientePessoaFisicaDTO clienteDTO) {
        ClientePessoaFisica cliente = convertToEntity(clienteDTO);
    ...
    public ResponseEntity<ClientePessoaFisicaDTO> updateCliente(@PathVariable int id, @RequestBody ClientePessoaFisicaDTO updatedClienteDTO) {
        for (ClientePessoaFisica cliente : clientes) {
            if (cliente.getIdCliente() == id) {
                ...
                cliente.setEndereco(updatedClienteDTO.getEndereço());
    ...
    public ResponseEntity<Boolean> login(@RequestBody ClientePessoaFisicaDTO clienteDTO) {
        for (ClientePessoaFisica storedCliente : clientes) {
    ...
    private ClientePessoaFisicaDTO convertToDto(ClientePessoaFisica cliente) {
        return new ClientePessoaFisicaDTO(cliente.getIdCliente(), cliente.getNome(), cliente.getEmail(), cliente.getEndereco(),
                cliente.getTelefone(), cliente.getUserName(), cliente.getPassword(), cliente.getCpf(), cliente.getDataNascimento(), cliente.getRg());
    }

    private ClientePessoaFisica convertToEntity(ClientePessoaFisicaDTO clienteDTO) {
        return new ClientePessoaFisica(clienteDTO.getIdCliente(), clienteDTO.getNome(), clienteDTO.getEmail(), clienteDTO.getEndereço(),
